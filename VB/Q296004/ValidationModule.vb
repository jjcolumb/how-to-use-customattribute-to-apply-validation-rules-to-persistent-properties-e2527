...
Namespace DXSample
    Public Class Validator
        ...
        Private Function ValidateUniqueRule(ByVal [property] As XPMemberInfo) As Boolean
            Dim val As Object = If(fCustomValues.ContainsKey([property].Name), fCustomValues([property].Name), [property].GetValue(fValidatedObject))
    ...
    Public Class CustomValidationEvaluatorContextDescriptor
        ...
        Private fCustomValue As EventHandler(Of CustomValueEventArgs)
        Public Custom Event CustomValue As EventHandler(Of CustomValueEventArgs)
            AddHandler(ByVal value As EventHandler(Of CustomValueEventArgs))
                fCustomValue = DirectCast(System.Delegate.Combine(fCustomValue, value), EventHandler(Of CustomValueEventArgs))
            End AddHandler
            RemoveHandler(ByVal value As EventHandler(Of CustomValueEventArgs))
                fCustomValue = DirectCast(System.Delegate.Remove(fCustomValue, value), EventHandler(Of CustomValueEventArgs))
            End RemoveHandler
            RaiseEvent(ByVal sender As System.Object, ByVal e As CustomValueEventArgs)
                If fCustomValue IsNot Nothing Then
                    For Each d As EventHandler(Of CustomValueEventArgs) In fCustomValue.GetInvocationList()
                        d.Invoke(sender, e)
                    Next d
                End If
            End RaiseEvent
